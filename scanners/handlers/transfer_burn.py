...
class HandlerTransferBurn(HandlerABC):
    ...
    def imported_mint_event(
        ...
        print("got into import mint")
        if TokenHistory.objects.filter(
            token__internal_id=token_id,
            method="Mint",
        ...
        token = Token.objects.filter(
            collection=collection, internal_id=token_id
        ).first()
        if not token:
            print("preparing to create token")
        ...
        print(token)
        ownership = Ownership.objects.filter(owner=owner, token=token).first()
        if not ownership:
            Ownership.objects.create(owner=owner, token=token, quantity=amount)
        else:
            ownership.quantity = ownership.quantity + amount
            ownership.save(update_fields=("quantity",))

        print("preparing to create history")
        TokenHistory.objects.get_or_create(
            token=token,
            tx_hash=tx_hash,
            method="Mint",
