...
class GameCompany(models.Model):
    ...
    videos = ArrayField(
        base_field=models.URLField(max_length=100), null=True, blank=True
    )
    video_covers_ipfs = ArrayField(
        base_field=models.CharField(max_length=60), size=3, null=True, blank=True
    )
    photos_ipfs = ArrayField(
        base_field=models.CharField(max_length=60), size=15, null=True, blank=True
    )
    # social medias
    developer = models.CharField(max_length=200, null=True, blank=True)
    release_status = models.CharField(max_length=200, null=True, blank=True)
    ...
    def photos(self) -> List[str]:
        if not self.photos_ipfs:
            return []
        return [
            f"https://{config.IPFS_DOMAIN}/ipfs/{ipfs_hash}"
            for ipfs_hash in self.photos_ipfs
        ]

    @photos.setter
    def photos(self, value: list) -> None:
        if value:
            self.photos_ipfs = value

    @property
    def video_covers(self) -> List[str]:
        if not self.video_covers_ipfs:
            return []
        return [
            f"https://{config.IPFS_DOMAIN}/ipfs/{ipfs_hash}"
            for ipfs_hash in self.video_covers_ipfs
        ]

    @video_covers.setter
    def video_covers(self, value: list) -> None:
        if value:
            self.video_covers_ipfs = value
...
class GameCategory(models.Model):
    ...
    cover_ipfs = models.CharField(max_length=100, null=True, blank=True)
    is_approved = models.BooleanField(null=True)
