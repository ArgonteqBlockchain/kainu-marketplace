...
from src.games.utils import base64_to_ipfs, get_photos_ipfs_list
...
class CreateView(APIView):
    ...
    def post(self, request):
        ...
        request_data["avatar"] = base64_to_ipfs(avatar)

        serializer = GameCompanyCreateSerializer(
            data=request_data, context={"network": network.id, "user": user.id}
        )
        serializer.is_valid()
...
class GetGameView(APIView):
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                ...
                "video_covers": openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Items(type=openapi.TYPE_STRING),
                ),
                "videos": openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Items(type=openapi.FORMAT_URI),
                ),
                ...
                "release_status": openapi.Schema(type=openapi.TYPE_STRING),
    ...
    def patch(self, request, game_name, network):
        ...
        request_data = request.data.copy()
        covers = request_data.pop("video_covers", [])
        videos = request.data.get("videos", [])

        if covers:
            video_covers = get_photos_ipfs_list(
                game_company, covers, "video_covers_ipfs"
            )
            request_data["video_covers"] = video_covers

            print(len(videos), len(video_covers))
            if (video_covers or videos) and len(videos) != len(video_covers):
                raise LengthMismatch

        photos = request.data.get("photos", [])
        if photos:
            photos = get_photos_ipfs_list(game_company, photos, "photos_ipfs")
            request_data["photos"] = photos

        serializer = GameCompanySerializer(
            game_company, data=request_data, partial=True
        )
...
class GetGameCategoryView(APIView):
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                ...
                "remove_avatar": openapi.Schema(type=openapi.TYPE_BOOLEAN),
                "remove_cover": openapi.Schema(type=openapi.TYPE_BOOLEAN),
                "subcategories": openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Items(
                        type=openapi.TYPE_OBJECT,
                        properties={
                            ...
                            "avatar": openapi.Schema(
                                type=openapi.FORMAT_BINARY, description="base64"
                            ),
    ...
    def patch(self, request, game_name, network, category_name):
        ...
        if request_data.get("cover"):
            request_data["cover_ipfs"] = base64_to_ipfs(request_data["cover"])
        for index, subcategory in enumerate(request_data.get("subcategories", [])):
            ...
            if subcategory.get("cover"):
                request_data["subcategories"][index]["cover_ipfs"] = base64_to_ipfs(
                    subcategory["cover"]
                )
        ...
        for subcategory in request_data.get("subcategories", []):
            ...
            if subcategory.get("remove_cover"):
                game_subcategory.cover_ipfs = None
                game_subcategory.save(update_fields=("cover_ipfs",))

        if request.data.get("remove_avatar"):
            game_category.avatar_ipfs = None
            game_category.save(update_fields=("avatar_ipfs",))
        if request.data.get("remove_cover"):
            game_category.cover_ipfs = None
            game_category.save(update_fields=("cover_ipfs",))
...
class GetGameSubCategoryView(APIView):
    ...
    @swagger_auto_schema(
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "name": openapi.Schema(type=openapi.TYPE_STRING),
                "avatar": openapi.Schema(
                    type=openapi.FORMAT_BINARY, description="binary file"
                ),
                "cover": openapi.Schema(
                    type=openapi.FORMAT_BINARY, description="binary file"
                ),
    ...
    def patch(self, request, game_name, network, category_name, subcategory_name):
        ...
        matching_game_subcategory = (
            GameSubCategory.objects.filter(
                name__iexact=request.data.get("name"),
                category=game_subcategory.category,
            )
            .exclude(is_approved=False)
        ...
        cover_base64 = request.data.get("cover")

        if request.data.get("remove_cover"):
            game_subcategory.cover_ipfs = None
        if cover:
            print("setting cover")
            cover_ipfs = send_to_ipfs(cover)
            game_subcategory.cover_ipfs = cover_ipfs
            print(game_subcategory.cover_ipfs)
        elif cover_base64:
            cover_ipfs = base64_to_ipfs(cover_base64)
            game_subcategory.cover_ipfs = cover_ipfs
